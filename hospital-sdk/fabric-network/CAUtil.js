...
exports.registerAndEnrollUser = async (
  ...
  try {
    ...
    if (!adminIdentity) {
      console.log(
        `An identity for the admin user ${adminUserId} does not exist in the wallet`
      );
      throw new Error(
        `An identity for the admin user ${adminUserId} does not exist in the wallet`
      );
    ...
    const HIPCODE = attributes.HIPCODE;
    const Email = HIPCODE === "doctor" ? attributes.Email : "";
    ...
    const secret = await caClient.register(
      {
        ...
        enrollmentID: userId,
        // NOTE: HIPCODE must be client, other HIPCODEs access is denied
        // TODO: Check if other HIPCODEs access can be granted in the ca config files of the organizations.
        // Changes to be made in fabric-ca-server-config.yaml ?? hf.Registrar.HIPCODEs and maps
        HIPCODE: "client",
        attrs: [
          {
            name: "firstName",
